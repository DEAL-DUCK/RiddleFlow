...
from core.models import db_helper, Group, User, Hackathon, Contest
...
async def user_is_owner_of_this_group_or_contest_creator(
    group: Group = Depends(get_group_by_id),
    user: User = Depends(current_active_user),
    contest: Contest = Depends(get_contest_by_id),
):
    if group.owner_id == user.id or user.id == contest.creator_id:
        return user
    raise HTTPException(
        status_code=status.HTTP_403_FORBIDDEN,
        detail=f"user {user.id} is not owner of group",
