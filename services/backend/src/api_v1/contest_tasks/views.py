...
from core.models import User, ContestTask, Contest
...
from api_v1.contests.dependencies import user_is_creator_of_this_contest, get_inactive_contest
...
async def create_task(
    ...
    return await crud.create_task_for_contest(
        session=session, task_data=task_data, contest_id=contest.id
    )
...
async def get_task_by_id(
    ...
    task = await crud.get_contest_task_by_id(session=session, task_id=task_id)
